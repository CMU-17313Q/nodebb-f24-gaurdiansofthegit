...
function initialize(req) {
  ...
  return Promise.all([user.isAdminOrGlobalMod(req.uid), user.getModeratedCids(req.uid), plugins.hooks.fire('filter:flags.validateFilters', {
...
function determineHasFilter(filters) {
  return !!Object.keys(filters).length && !(Object.keys(filters).length === 1 && filters.hasOwnProperty('page')) && !(Object.keys(filters).length === 2 && filters.hasOwnProperty('page') && filters.hasOwnProperty('perPage'));
}
