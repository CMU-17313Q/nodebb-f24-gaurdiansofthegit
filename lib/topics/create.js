...
module.exports = function (Topics) {
  ...
  Topics.post = async function (data) {
    ...
    const [categoryExists, canCreate, canTag, isAdmin] = await Promise.all([categories.exists(data.cid), privileges.categories.can('topics:create', data.cid, uid), privileges.categories.can('topics:tag', data.cid, uid), privileges.users.isAdministrator(uid)]);
  ...
  async function onNewPost(postData, data) {
    ...
    const [userInfo, topicInfo] = await Promise.all([posts.getUserInfoForPosts([postData.uid], uid), Topics.getTopicFields(tid, ['tid', 'uid', 'title', 'slug', 'cid', 'postcount', 'mainPid', 'scheduled', 'tags']), Topics.addParentPosts([postData]), Topics.syncBacklinks(postData), posts.parsePost(postData)]);
